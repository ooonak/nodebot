...
void ok::SlashCommandController::start(dpp::snowflake &guildId)
{
  if (!mInitialized)
  {
    ...
    auto idOption = dpp::command_option(dpp::co_integer, "id", "The ID of the node as dislayed in it's embed message.", true);
    idOption.set_min_value(dpp::command_option_range{0});
    cmdCommand.add_option(idOption);

    auto cmdOption = dpp::command_option(dpp::co_string, "cmd", "Command to invoke.", true);
    cmdOption.set_max_length(dpp::command_option_range{256});
    cmdCommand.add_option(cmdOption);

    for (size_t i = 0; i < NumberOfArgs; ++i)
    {
      auto argOption = dpp::command_option(dpp::co_string, "arg" + std::to_string(i), "Optional argument " + std::to_string(i + 1) + "to command.", false);
      argOption.set_max_length(dpp::command_option_range{256});
      cmdCommand.add_option(argOption);
    }

    mBot->guild_command_create(helpCommand, guildId, std::bind(&SlashCommandController::onCommandCreated, this, _1));
    mBot->guild_command_create(cmdCommand, guildId, std::bind(&SlashCommandController::onCommandCreated, this, _1));
  }
}
...
void ok::SlashCommandController::onSlashCommand(const dpp::slashcommand_t &event)
{
  ...
  else if (event.command.get_command_name() == "cmd")
  {
    ...
    if (itVec != mNodes.end())
    {
      ...
      {
        event.reply("Node " + std::to_string(id) + ", has no callback registered for command '" + cmdName + "'.");
      }
    }
    else
    {
      event.reply("Unknown node " + std::to_string(id) + ", valid values are 0-" + std::to_string(mNodes.size()) + ".");
    }
  }
}
